...
## Introduction
React Aria components such as [Link](/docs/3.x/components/navigation/link), [Menu](/docs/3.x/components/collections/menu), [Tabs](/docs/3.x/components/navigation/tabs), and [Table](/docs/3.x/components/collections/table) transform elements into clickable links that navigate when clicked. These components utilize the `href` prop to render as an `<a>` tag, supporting attributes like `target` and `download`.
