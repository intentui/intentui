...
## Introduction
React Aria components such as [Link](/docscomponents/navigation/link), [Menu](/docscomponents/collections/menu), [Tabs](/docscomponents/navigation/tabs), and [Table](/docscomponents/collections/table) transform elements into clickable links that navigate when clicked. These components utilize the `href` prop to render as an `<a>` tag, supporting attributes like `target` and `download`.
