...
We also added a new `useMobile` hook to make it easier to detect mobile devices, replacing the previous `useMediaQuery` hook. With that, all usage of useMediaQuery in the core components has been update to `useMobile`.
